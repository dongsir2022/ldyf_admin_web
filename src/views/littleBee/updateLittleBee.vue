<template>
  <div class="app-container">
    <el-form ref="form" :model="form" :rules="rules" label-width="100px" label-position="top">
      ...
      <el-form-item>
        <el-button type="primary" :loading="loading" @click="onSubmit" plain>更新</el-button>
...
<script>
import {getLittleBeeInfo, updateLittleBee} from "@/api/littleBee";
...
export default {
  ...
  created() {
    if (isNotBlank(this.$route.params.id)) {
      this.form.id = this.$route.params.id;
      this.fetchData();
    } else {
      this.$message({
        message: this.$t('alert.dataError'),
        type: 'error',
        duration: 1000
      })
  ...
  methods: {
    fetchData() {
      getLittleBeeInfo(this.form.id).then(res => {
        this.form = res.data;
        if (isNotBlank(this.form.location_desc)) {
          let descArr = this.form.location_desc.split("|");
          let locationArr = [];
          this.form.location = [];
          console.log(descArr);
          locationArr.push(TextToCode[descArr[0]].code);
          if (descArr.length > 1 && isNotBlank(descArr[1])) {
            locationArr.push(TextToCode[descArr[0]][descArr[1]].code);
            if (descArr.length > 2 && isNotBlank(descArr[2])) {
              locationArr.push(TextToCode[descArr[0]][descArr[1]][descArr[2]].code);
            } else {
              locationArr.push("");
            }
          } else {
      ...
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          let data = {
            ...
            email: isNotBlank(this.form.email) ? this.form.email : '',
            password: isNotBlank(this.form.password) ? this.form.password : ''
          }
          updateLittleBee(data).then(res => {
            this.loading = false;
            this.$message({
